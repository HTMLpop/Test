...
async def check_url(browser, url: str, timeout_ms: int = 25000, headed: bool = False, screenshot_dir: Path | None = None) -> dict:
    ...
    try:
        ...
        # Try to wait for network to be idle; if it times out, we still proceed
        try:
        ...
        body_text = ""
        try:
            # limit large text to keep memory down
        ...
        # Decide status
        # If we got 200-299 or 3xx and not a challenge page and has some content => reachable
        if (result["http_status"] and 200 <= result["http_status"] < 400) or result["http_status"] is None:
            # If explicitly looks like a challenge wall
            if classification == "challenge":
                ...
                if text_len > 200 or has_h1 or (result["title"] and len(result["title"]) > 0):
                    ...
                    # Not enough content, could be protected or a minimal page
                    result["status"] = "protected" if classification == "protected" else "reachable"
        else:
            # HTTP error statuses
            if result["http_status"] in (401, 403):
                # Could be bot block or auth required; mark protected/challenge accordingly
        ...
        if screenshot_dir:
            ...
            fname = re.sub(r"[^a-zA-Z0-9_-]+", "_", (urlparse(result["final_url"] or url).netloc or "page"))
        ...
        result["notes"] += msg
        if "ERR_NAME_NOT_RESOLVED" in msg or "dns" in msg.lower():
            result["status"] = "dns_error"
        else:
            result["status"] = "broken"
    finally:
        result["elapsed_sec"] = round(time.time() - start, 2)
        # Ensure page/context cleanup
        try:
